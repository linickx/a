...
if (!isset($_GET['oauth_verifier'])) {
    ...
    */
    ...
    if (!$uid) { // new user
    	...
    	$db->query("INSERT INTO user (id, tw_at, tw_sec) VALUES (NULL,\"$access_token\",\"$access_token_secret\")");
		$uid = $db->insert_id;
    ...
    setcookie("rxalert[uid]", $uid, time() + 86400);
    setcookie("rxalert[at]", $access_token, time() + 86400);

    // Generate a checksum to authenticate user.
    $CookieAuth = "nb_" . $uid . SALT . $access_token;
    $CookieAuth = sha1($CookieAuth);
	setcookie("rxalert[auth]", $CookieAuth, time() + 86400);
